...
const Sessions = (() => {
  ...
  async function _createSessionInProject(project) {
    const opts = { existingSessions: _sessions, useDefaults: true };
    if (project && project.path) {
      opts.workingDir = project.path;
      opts.agentProfile = "coding";
    }
    const session = await NewSessionStore.createSession(opts);
    if (!session) return null;
    NewSessionStore.reset();
    if (!_sessions.find(s => s.id === session.id)) {
      _sessions.unshift(session);
    }
    if (project && project.id && window.ProjectsStore) {
      ProjectsStore.setActive(project.id, { persist: true, touch: true });
      ProjectsStore.setCollapsed(project.id, false);
    }
  ...
  async function _openProjectFlow() {
    if (!window.ProjectsStore) {
      alert("Projects are unavailable");
      return null;
    }
    try {
      const project = await ProjectsStore.openWithPicker();
      if (!project) return null;
      Sessions.renderList();
      return project;
    } catch (e) {
      alert((I18n.t("projects.openError") || "Failed to open project: ") + (e && e.message ? e.message : e));
      return null;
    }
  }

  function _initNewSessionControls() {
    // Main button: create in the active project when one is selected; otherwise
    // keep the historical general session in the default workspace.
    ...
    _btnNewInline.addEventListener("click", async () => {
        ...
        try {
          const active = window.ProjectsStore ? ProjectsStore.activeProject() : null;
          await _createSessionInProject(active);
        } finally {
          _btnNewInline.disabled = false;
        }
      });

    const openProjectMenu = document.getElementById("btn-new-session-open-project");
    if (openProjectMenu) {
      openProjectMenu.addEventListener("click", async () => { await _openProjectFlow(); });
    }
    ...
    document.addEventListener("click", (e) => {
      ...
      const newInProject = e.target && e.target.closest && e.target.closest("[data-action='new-in-project']");
      if (newInProject) {
        e.preventDefault();
        e.stopPropagation();
        const projectId = newInProject.dataset.projectId;
        const project = window.ProjectsStore
          ? ProjectsStore.list().find(p => p.id === projectId)
          : null;
        if (project) _createSessionInProject(project);
      }
      const toggleMore = e.target && e.target.closest && e.target.closest("[data-action='toggle-project-more']");
      if (toggleMore) {
        e.preventDefault();
        e.stopPropagation();
        const projectId = toggleMore.dataset.projectId;
        if (window.ProjectsStore && projectId) {
          ProjectsStore.toggleExpandedMore(projectId);
          Sessions.renderList();
        }
      }
    });
  ...
  function _renderProjectGroup(container, group) {
    const project = group.project;
    const collapsed = !!group.collapsed;
    const isActive = !!group.active;
    const allSessions = group.sessions || [];
    const limit = (window.ProjectsStore && ProjectsStore.sessionPreviewLimit) || 5;
    let expandedMore = !!(window.ProjectsStore && ProjectsStore.isExpandedMore(project.id));
    const hasHidden = allSessions.length > limit;
    // If the open session sits past the preview window, expand for this render
    // so the active item remains visible without forcing a permanent preference.
    if (hasHidden && !expandedMore && _activeId && allSessions.slice(limit).some((s) => s.id === _activeId)) {
      expandedMore = true;
    }
    const visibleSessions = (!collapsed && hasHidden && !expandedMore)
      ? allSessions.slice(0, limit)
      : allSessions;
    const hiddenCount = Math.max(0, allSessions.length - limit);

    const wrap = document.createElement("div");
    wrap.className = "project-group" + (isActive ? " active" : "") + (collapsed ? " collapsed" : "");
    wrap.dataset.projectId = project.id;

    const moreBtnHtml = hasHidden
      ? `<button type="button" class="btn-icon-sm" data-action="toggle-project-more" data-project-id="${escapeHtml(project.id)}" title="${escapeHtml(expandedMore ? I18n.t("projects.showLess") : I18n.t("projects.showMore", { n: hiddenCount }))}">
    ...
    header.innerHTML = `
      <button type="button" class="project-group-toggle" aria-label="Toggle project">
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <path d="${collapsed ? "M3.5 2L6.5 5L3.5 8" : "M2 3.5L5 6.5L8 3.5"}" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </button>
      <div class="project-group-main">
        <div class="project-group-name">${escapeHtml(project.name || project.path || "Project")}</div>
        <div class="project-group-path">${escapeHtml(project.path || "")}</div>
      </div>
      <div class="project-group-actions">
        <button type="button" class="btn-icon-sm" data-action="new-in-project" data-project-id="${escapeHtml(project.id)}" title="${escapeHtml(I18n.t("projects.newChat"))}">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    ...
    header.addEventListener("click", (e) => {
      if (e.target.closest(".project-group-actions")) return;
      if (!window.ProjectsStore) return;
      if (e.target.closest(".project-group-toggle")) {
        ProjectsStore.toggleCollapsed(project.id);
      } else {
        ProjectsStore.setActive(project.id, { persist: true, touch: true });
        ProjectsStore.setCollapsed(project.id, false);
      }
    ...
    header.addEventListener("contextmenu", (e) => {
      e.preventDefault();
      e.stopPropagation();
      Sessions._showProjectContextMenu(e, project);
    });
    ...
    if (!collapsed) {
      ...
      if (allSessions.length === 0) {
        ...
        empty.textContent = I18n.t("projects.emptySessions");
        body.appendChild(empty);
      } else {
        visibleSessions.forEach((s) => _renderSessionItem(body, s));
        if (hasHidden && !expandedMore) {
          const more = document.createElement("button");
          more.type = "button";
          more.className = "project-group-more";
          more.dataset.action = "toggle-project-more";
          more.dataset.projectId = project.id;
          more.textContent = I18n.t("projects.showMore", { n: hiddenCount });
  ...
  function _renderUngroupedHeader(container) {
    const el = document.createElement("div");
    el.className = "project-ungrouped-label";
    el.textContent = I18n.t("projects.ungrouped");
    container.appendChild(el);
  }
  ...
  return {
    ...
    init() {
      ...
      _initMessageHistory();
      const boot = [];
      boot.push(_ensureAgentsLoaded());
      if (window.ProjectsStore) {
        boot.push(ProjectsStore.load());
        ProjectsStore.on("projects:changed", () => Sessions.renderList());
        ProjectsStore.on("projects:active-changed", () => Sessions.renderList());
      }
      Promise.all(boot).then(() => Sessions.renderList());
    ...
    select(id) {
      const s = _sessions.find(s => s.id === id)
             || _extraSessions.find(s => s.id === id)
             || _searchResults.find(s => s.id === id);
      if (!s) return;
      if (_searchOpen) Sessions.toggleSearch();   // close palette on pick
      if (window.ProjectsStore && s.working_dir) {
        const project = ProjectsStore.findByPath(s.working_dir);
        if (project) {
          // Selecting a session counts as project activity (MRU).
          ProjectsStore.setActive(project.id, { persist: true, touch: true });
          ProjectsStore.setCollapsed(project.id, false);
        }
      }
    ...
    renderList({ scrollToActive = false } = {}) {
      ...
      if (isCronView) {
        ...
        const grouped = window.ProjectsStore
          ? ProjectsStore.groupSessions(nonCron)
          : { groups: [], ungrouped: nonCron };

        // Project folders first (Codex/Claude-style).
        grouped.groups.forEach((group) => _renderProjectGroup(list, group));

        // Cron virtual entry + ungrouped sessions after projects.
        const renderUngroupedWithCron = (rows) => {
          if (_cronCount > 0) {
        ...
        if (grouped.groups.length > 0 && grouped.ungrouped.length > 0) {
          _renderUngroupedHeader(list);
        }
        renderUngroupedWithCron(grouped.ungrouped);

        if (grouped.groups.length === 0 && grouped.ungrouped.length === 0 && _cronCount === 0) {
    ...
    /** Shared session action items used by right-click and ··· menus. */
    _sessionActionIcons() {
      return {
        ...
        fork: `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="6" y1="3" x2="6" y2="15"/><circle cx="18" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M18 9a9 9 0 0 1-9 9"/></svg>`,
        rename: `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4z"/></svg>`,
    ...
    _buildSessionActionsHtml(session) {
      const icons = Sessions._sessionActionIcons();
      ...
      return `
        <div class="session-actions-menu-item" data-action="fork">
          <span class="session-actions-menu-icon">${icons.fork}</span>
        ...
        <div class="session-actions-menu-item session-actions-menu-item--danger" data-action="delete">
          <span class="session-actions-menu-icon">${icons.trash}</span>
          <span class="session-actions-menu-label">${escapeHtml(I18n.t("sessions.actions.delete"))}</span>
        </div>
      `;
    },

    async _runSessionAction(action, session) {
      if (action === "fork") {
    ...
    /** Show right-click context menu for a session item (same actions as ···). */
    _showContextMenu(e, session) {
      ...
      menu.innerHTML = Sessions._buildSessionActionsHtml(session);
      ...
      menu.addEventListener("click", async (ev) => {
        ...
        Sessions._closeContextMenu();
        await Sessions._runSessionAction(action, session);
      });
    ...
    /** Right-click menu on a project folder header (rename / remove). */
    _showProjectContextMenu(e, project) {
      Sessions._closeContextMenu();
      Sessions._closeActionsMenu();
      ...
      menu.className = "session-context-menu";
      menu.innerHTML = `
        <div class="session-actions-menu-item" data-action="rename-project">
          <span class="session-actions-menu-icon">${icons.rename}</span>
          <span class="session-actions-menu-label">${escapeHtml(I18n.t("projects.rename"))}</span>
        </div>
        <div class="session-actions-menu-item session-actions-menu-item--danger" data-action="remove-project">
          <span class="session-actions-menu-icon">${icons.trash}</span>
          <span class="session-actions-menu-label">${escapeHtml(I18n.t("projects.remove"))}</span>
        </div>
      `;
      ...
      menu.addEventListener("click", async (ev) => {
        ...
        if (action === "rename-project") {
          ...
          const next = (typeof Modal.rename === "function")
            ? await Modal.rename(current)
            : await Modal.prompt(I18n.t("projects.renamePrompt"), current);
          if (next == null) return;
          const name = String(next).trim();
          if (!name || name === current) return;
          try {
            await ProjectsStore.rename(project.id, name);
            Sessions.renderList();
          } catch (err) {
            alert((I18n.t("projects.renameError") || "Failed to rename project: ") + err.message);
          }
          return;
        }

        if (action === "remove-project") {
          const label = project.name || project.path || project.id;
          const ok = await Modal.confirm(I18n.t("projects.removeConfirm", { name: label }));
          if (!ok) return;
          try {
            await ProjectsStore.remove(project.id);
            Sessions.renderList();
          } catch (err) {
            alert((I18n.t("projects.removeError") || "Failed to remove project: ") + err.message);
          }
        }
      });
